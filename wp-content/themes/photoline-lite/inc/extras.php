...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	 ...
	 */
	function photoline_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( __( 'Page %s', 'photoline' ), max( $paged, $page ) );
		}

		return $title;
	}
	add_filter( 'wp_title', 'photoline_wp_title', 10, 2 );
	 ...
	 */
	function photoline_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
	add_action( 'wp_head', 'photoline_render_title' );
endif;