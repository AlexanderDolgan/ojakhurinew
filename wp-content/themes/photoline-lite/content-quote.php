...
echo '<cite>' . $cite . ':</cite>';
?>
<div<?php $thumbnail = wp_get_attachment_image_src( get_post_thumbnail_id(), 'photoline-aside' ); ?> style="margin-bottom:10px;padding-top:20px;color:#FFF;background:<?php echo get_theme_mod( 'photoline_link_color', '#2d2d2d' ); ?><?php if  ( $thumbnail ) { ?> url(<?php echo $thumbnail[0]; ?>) no-repeat; background-position: 50%; background-size: cover<?php } ?>;">
<?php
echo $content . '</div>';
?>
		</div><!-- .entry-content -->
